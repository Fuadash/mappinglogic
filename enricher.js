...
function normalizeForSemanticDetection(value) {
    return value
        .replace(/[@.,;:'"()\-_/\\]/g, " ")
        .replace(/\s+/g, " ")
        .trim();
...
function detectSemanticType(value) {
    ...
    // pure number (still strict)
    if (/^[0-9]+(\.[0-9]+)?$/.test(raw)) return "number";

    const cleaned = normalizeForSemanticDetection(raw);

    const hasLetters = /[a-zA-Z]/.test(cleaned);
    const hasNumbers = /[0-9]/.test(cleaned);

    if (hasLetters && hasNumbers) return "numbers_and_letters";
    if (hasLetters) return "letters";
